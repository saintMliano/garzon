...
- **Suscripción (F10):** un solo plan ($29.900/mes), la cobra el dueño por fuera y el super-admin registra `suscripcion_hasta`. La regla vive **solo** en `situacion_suscripcion(estado, hasta)`: si necesitás saber si un local está al día, llamala — **no recalcules fechas en otro lado**. Reglas que no se negocian: falla **hacia abierto** (sin fecha = al día), hay **7 días de gracia** antes de cortar, el corte real está en `crear_pedido` (el navegador solo acompaña), la carta **se sigue viendo** pausada, el **dashboard y los reportes nunca se bloquean**, y el comensal **nunca** se entera del motivo (hay un test que falla si el mensaje lo delata). Las columnas `plan`/`suscripcion_*` no tienen `GRANT UPDATE` para `authenticated`: se escriben solo por `/api/admin/suscripcion`.
